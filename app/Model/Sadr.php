...
function sendSubmitEmail() {
    ...
    $sadrs = $this->find('list', array(
        'fields' => array('Sadr.id', 'Sadr.reporter_email'),
        'conditions' => array('Sadr.emails' => 1, 'Sadr.submitted' => array(1,2)),
        'limit' => Configure::read('Emails.limit'),
        'recursive' => -1
      ));
    $email = new CakeEmail();
    foreach ($sadrs as $key => $val) {
      ...
      if($this->saveField('emails', 2)){
        $email->config('gmail');
        $email->template('submitted_sadr');
        $email->emailFormat('html');
        $email->to($val);
        $email->cc(array('pv@pharmacyboardkenya.org', 'info@pharmacyboardkenya.org'));
        $email->subject(Configure::read('Emails.submitted_sadr.subject').$key);
        $email->viewVars(array('ID' => $key, 'root' => Configure::read('Domain.root')));
        if(!$email->send()){
          $this->log($email, 'debug');
        }
