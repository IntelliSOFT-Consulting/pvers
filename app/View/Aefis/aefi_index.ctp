					  ...
					  <div class="well" style="padding: 8px 0;">
						<ul class="nav nav-list">
						  ...
						  <li class="<?php if(!isset($this->request->params['named']['submit'])) echo 'active';?>">
							<?php echo $this->Html->link('<i class="icon-book"></i> All AEFI Reports', array('controller' => 'aefis', 'action' => 'aefiIndex'), array('escape' => false)); ?>
						  </li>
						  <li class="<?php if(isset($this->request->params['named']['submit']) && $this->request->params['named']['submit'] == '2') echo 'active';?>">
							<?php echo $this->Html->link('<span class="badge badge-success">1</span> Submitted',
								array('controller' => 'aefis', 'action' => 'aefiIndex', 'submit' => '2'), array('escape' => false)); ?>
						  </li>
						  <li class="<?php if(isset($this->request->params['named']['submit']) && $this->request->params['named']['submit'] == '0') echo 'active';?>">
							<?php echo $this->Html->link('<span class="badge badge-important">2</span> UnSubmitted',
								array('controller' => 'aefis', 'action' => 'aefiIndex', 'submit' => '0'), array('escape' => false)); ?>
						  </li>
						  <li class="divider"></li>
						  <li>
							<?php echo $this->Html->link('<i class="icon-tag"></i> Institution Reports (AEFI)', array('controller' => 'aefis', 'action' => 'institutionCodes'), array('escape' => false)); ?>
						  </li>
						  <li class="divider"></li>
						  <li>
		...
		<div class="span10 columns">
				<?php
						echo $this->Form->create('Aefi', array(
							'url' => array_merge(array('action' => 'aefiIndex'), $this->params['pass']),
							'class' => 'well',
						));
					?>
					<div class="row-fluid">
						...
						<?php
							...
							echo $this->Form->input('pages', array(
								'type' => 'select', 'div' => false, 'class' => 'span9', 'selected' => $this->request->params['paging']['Aefi']['limit'],
								'empty' => true,
								'options' => array(
													5=>5,
													10=>10,
						...
						<?php
							echo $this->Form->input('patient_name',
									array('div' => false, 'id' => 'adminPatientId',
										'class' => 'span9', 'label' => array('class' => 'required', 'text' => 'Patient Name')));
						?>
						</div>
						<div class="span3 columns">
						<?php
							...
							echo $this->Form->input('end_date',
								array('div' => false, 'type' => 'text', 'class' => 'input-small',
									   'after' => '<a style="font-weight:normal" onclick="$(\'#AefiStartDate\').val(\'\');	$(\'#AefiEndDate\').val(\'\');
														$(\'#adminSearchId\').val(\'\');	$(\'#adminPatientId\').val(\'\');" >
													<em>clear</em></a>',
					...
					<?php
						...
						foreach ($aefis as $aefi): ?>
							<tr>
								...
								<td><?php
									if($aefi['Aefi']['submitted'] > 1) echo '<span class="badge badge-success">'.$aefi['Aefi']['id'].'</span>';
									else  echo '<span class="badge badge-important">'.$aefi['Aefi']['id'].'</span>';
									?>&nbsp;
								</td>
